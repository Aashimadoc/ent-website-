...
function BlogsPage() {
  return <>
    ...
    <section className="section bg-mist">
      <div className="shell">
        <div className="grid gap-6 md:grid-cols-3">{topics.map((t)=><Card key={t.title}><t.icon className="size-8 text-primary"/><h2 className="mt-5 text-xl font-semibold text-navy">{t.title}</h2><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{t.body}</p><Link to={t.to} className="mt-5 inline-flex text-sm font-semibold text-primary hover:underline">Read related information</Link></Card>)}</div>
        <div className="mt-10 rounded-3xl border border-border bg-card p-6 shadow-card md:p-8">
          <p className="eyebrow">Featured guide</p>
          <h2 className="mt-3 text-2xl font-semibold text-navy">Snoring: When Should You See an ENT Specialist?</h2>
          <p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground md:text-base">Occasional snoring is common, but persistent loud snoring, witnessed breathing pauses or daytime sleepiness can justify an ENT and sleep assessment.</p>
          <Link to="/blogs/snoring-when-to-see-an-ent-specialist" className="mt-5 inline-flex text-sm font-semibold text-primary hover:underline">Read the patient guide</Link>
        </div>
      </div>
    </section>
    <CtaBand />
